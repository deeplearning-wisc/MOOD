...
class FlowStep(nn.Module):
    ...
    def normal_flow(self, input, logdet):
        ...
        #print(z.shape)
        #print(logdet)
        ops = ops + input.numel()*2
        # 2. permute
        z, logdet = self.flow_permutation(z, logdet, False)
        #print( '# 2. permute' )
        #print(z.shape)
        #print(logdet)
        ops = ops + z.shape[2]*z.shape[3] *z.shape[1] *(1*1*z.shape[1] ) 
        # 3. coupling
        ...
        if self.flow_coupling == "additive":
            ...
            h = self.block(z1)
            #print(z1.shape[1])
            #print(self.hidden_channels,z1.shape[2],z1.shape[3])
            ops = ops + z1.shape[2]*z1.shape[3] *self.hidden_channels *(3*3*self.in_channels//2 )
            ops = ops + z1.shape[2]*z1.shape[3] *self.hidden_channels *3 # actnorm relu
            ops = ops + z1.shape[2]*z1.shape[3] *self.hidden_channels *(1*1*self.hidden_channels)
            ops = ops + z1.shape[2]*z1.shape[3] *self.hidden_channels *3 # actnorm relu
            ops = ops + z1.shape[2]*z1.shape[3] *self.in_channels     *(3*3*self.hidden_channels)
            #print(ops)
            shift, scale = split_feature(h, "cross")
            scale = torch.sigmoid(scale + 2.0)
            ops = ops + scale.numel()*(1+3)
            z2 = z2 + shift
            z2 = z2 * scale
            ops = ops + z2.numel()
