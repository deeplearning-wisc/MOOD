...
class down_shifted_conv2d(nn.Module):
    ...
    def forward(self, x):
        #print('down_shifted_conv2d')
        ops = 0
        x = self.pad(x)
        x = self.conv(x)
        ho, wo = x.shape[2], x.shape[3] 
        ops = ops + ho*wo*self.num_filters_out * (self.filter_size[0]*self.filter_size[1]*self.num_filters_in)
...
class down_shifted_deconv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        ho, wo = x.shape[2], x.shape[3] 
        x = self.deconv(x)
        ops = ops + ho*wo*self.num_filters_in * (self.filter_size[0]*self.filter_size[1]*self.num_filters_out)
...
class gated_resnet(nn.Module):
    def __init__(self, num_filters, conv_op, nonlinearity=concat_elu, skip_connection=0):
        ...
        if skip_connection != 0 : 
            ...
            self.nin_i = 2 * skip_connection * num_filters
            self.nin_o = num_filters
    ...
    def forward(self, og_x, a=None):
        ...
        if a is not None : 
            ...
            ops = ops + self.nonlinearity(a).numel()*self.nin_i
